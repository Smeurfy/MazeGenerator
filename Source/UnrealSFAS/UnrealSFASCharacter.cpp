...
AUnrealSFASCharacter::AUnrealSFASCharacter()
{
	...
	//Scene capture boom
	sceneCaptureBoom = CreateDefaultSubobject<USpringArmComponent>(TEXT("SceneCameraBoom"));
	sceneCaptureBoom->SetupAttachment(RootComponent);
	sceneCaptureBoom->TargetArmLength = 10000.0f; // The camera follows at this distance behind the character	

	//Scene capture camera
	sceneCaptureCamera = CreateDefaultSubobject<USceneCaptureComponent2D>(TEXT("SceneCaptureCamera"));
	sceneCaptureCamera->SetupAttachment(sceneCaptureBoom, USpringArmComponent::SocketName);

	//First person camera
	firstPersonCamera = CreateDefaultSubobject<UCameraComponent>(TEXT("FirstPersonCamera"));
	firstPersonCamera->SetupAttachment(ACharacter::GetMesh(), ACharacter::GetMesh()->GetBoneName(ACharacter::GetMesh()->GetBoneIndex("head")));
		//->GetAllSocketNames().Find("head"));
